...
In this vignette we show how to utilize the metadata information by drilling into NAICS categories.
...
There are a lot of NAICS categories in this dataset.
```{r}
data$`North American Industry Classification System (NAICS)` %>% unique %>% length
```
...
```{r}
data <- data %>% left_join((.) %>% filter(`North American Industry Classification System (NAICS)` %in%
