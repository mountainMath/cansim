...
tables %>% select("cansim_table_number","title")
```

The search came up with two tables, we choose the first one.

```{r}
table = 14100293
data <-get_cansim(table) %>% 
...
ggplot(data,aes(x=Date,group=GEO,y=Estimate)) +
  ...
  labs(y="Unemployment Rate",x="",color="Economic Region",caption=paste0("CANSIM ",table))
```
