...
We see that data set come with three different measures and 307 different NAICS values. Let's load the data and focus on just "`r if (exists("table_id", inherits=FALSE)) unique(dplyr::filter(get_cansim(table_id),grepl("Chained",.data$Prices))$Prices) else "Chained (2017) dollars"`". 
```{r}
...
```
There are still `r if (exists("n", inherits=FALSE)) n else 22` level 1 categories, too many to sensibly visualize at the same time. We can use `dplyr` functions to identify the top categories and group the rest so that we can have a plot that is easier to understand. 
